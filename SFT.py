...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):      # (X, Y, loss_mask)和PretrainDataset中的__getitem__有关
        ...
        if (step + 1) % args.save_step == 0:
            ...
            ckp = f'{args.save_dir}/SFT.pth'                  # 保存模型的路径，这里采用的是一直把模型保存到同一个文件中，而不是保存很多个模型
...
if __name__ == '__main__':    # 只有当前文件执行，才会执行，而不是被import后就执行
    ...
    parser.add_argument("--batch_size", type=int, default = 84)        # 每批次的样本数量
    ...
    parser.add_argument("--wandb_project", type=str, default="LLM-zyj-SFT")  # wandb项目名称
    ...
    parser.add_argument("--data_path", type=str, default="sft_512.jsonl")  # 训练数据的路径
