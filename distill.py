...
def init_model(llm_config):
    ...
    model = LLMzyj(llm_config)
    ckp = './results/SFT_1024.pth'                                    # 初始化模型
    state_dict = torch.load(ckp, map_location = args.device)          # 把模型参数取到字典中，并加载到GPU上
    # model.load_state_dict(state_dict, strict = False)               # strict 表示 是否严格匹配所有键
...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):      # (X, Y, loss_mask)和PretrainDataset中的__getitem__有关
        ...
        with ctx:                                         # 混合精度训练
            ...
            sp_ids = torch.isin(Y.view(-1), torch.tensor(start_of_think_ids + end_of_think_ids + start_of_answer_ids + end_of_answer_ids).to(args.device)) # 找到Y中含有特殊字符token_id的位置
        ...
        if (step + 1) % args.save_step == 0:
            ...
            ckp = f'{args.save_dir}/distill.pth'                  # 保存模型的路径，这里采用的是一直把模型保存到同一个文件中，而不是保存很多个模型
...
if __name__ == '__main__':    # 只有当前文件执行，才会执行，而不是被import后就执行
    ...
    parser.add_argument("--batch_size", type=int, default = 28)          # 每批次的样本数量
    ...
    parser.add_argument("--wandb_project", type=str, default="LLM-zyj-distill")  # wandb项目名称
    parser.add_argument("--num_workers", type=int, default=1)          # 数据加载时的CPU数
    parser.add_argument("--accumulation_steps", type=int, default = 8)   # 梯度累积步数，累积多少步更新一次，变相扩大了batch_size，更新梯度时，为batch_size * K个样本的梯度的均值
    parser.add_argument("--grad_clip", type=float, default = 1.0)        # 梯度裁剪阈值，防止梯度过大，提高训练稳定性（超过1.0的直接设置为1.0）
    parser.add_argument("--warmup_iters", type=int, default = 1700)         # warmup，即初始时，前多少步学习率缓慢线性增长
    ...
    parser.add_argument('--max_seq_len', default=1024, type=int)        # 输入的最大序列长度
    parser.add_argument("--data_path", type=str, default="r1_1024.jsonl")  # 训练数据的路径
    ...
    # 设置wandb运行名称
    args.wandb_run_name = f"LLM-zyj-distill-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
    ...
    train_ds =SFTDataset(args.data_path, tokenizer, max_length = lm_config.max_seq_len)  # 加载训练数据集
